...
def sort_files_by_date(path_to_files):
    ...
    Parameters:
    ----------
            path : The complete path to the desired directory containing the files.
                NOTE: it is assumed that all csv files in the directory 
                are names according to the date theiy were created with format
                ‘mm-dd-YYYY.csv’
    Returns:
    -------
...
def Q3(path_to_files):
    '''NOTE: Only part (a) was able to be completed, part b has been commented out.
    This function calculates and returns the total number of cases and deaths worldwide for each 
    date in the file directory

    Parameters
    ----------
    path_to_files : The complete path to the directory containing the files. A string.

    Returns
    -------
    Prints a string containing the data, number of new worldwide cases and number of new worldwide deaths for each of the files
    in the directory except for the oldest one (since it was not possible to calculate the new cases for this file)
    ...
    for i in range(0,(no_of_files-1)):
        ...
        #####Part b)
### For this part, the datetime module was used to find the day that corresponded to the date
# of the file name for all files in the directory except for the oldest one. If the date
#fell on a Sunday, the value was'0', '1' for Monday and so on. For each key in
#the date_cases_deaths_dict above, this day value was added to the list of values such that 
#each key was of form [new_cases,new_deaths,day].
...
### A new, empty dictionary was then created, weekly_cases_deaths, to store the total cases and deaths
#for each week. 
# The variable i corresponded to the position of the file_name in the tuple of sorted file names that
#forms the output of sort_files_by_date function. As such, it was initially set to -2, so that the second
#oldest file was the first to be analysed. After each loop, i = i-1 meant the next oldest was analysed
#next until all files expect the oldest had been analysed.
#A key was established for weekly_cases_deaths. If the day of the file being analysed was not 0 (ie not Sunday),
#then the new cases and new deaths were added to the existing values for weekly_cases_deaths[n].
#If the day was 0, the new cases and deaths were added to this key, but then a new key was
#forged so that the next file to be analysed was in the key corresponding to the next week.
##This function was unable to be completed. 
  ...
  #           weekly_cases_deaths[n] = [weekly_cases_deaths[n].values()[0] + date_cases_deaths_dict[date_name].values()[0], weekly_cases_deaths[n].values()[1] + date_cases_deaths_dict[date_name].values()[1]]
  #      else:
  #          weekly_cases_deaths[n] += [weekly_cases_deaths[n].values()[0] + date_cases_deaths_dict[date_name][0].values()[0], weekly_cases_deaths[n].values()[1] + date_cases_deaths_dict[date_name].values()[1]]
  #          n = n+1
  #      i = i - 1
...
if __name__ == '__main__':
   path_to_files = r'C:\Users\steve\OneDrive\Desktop\COMP1730 Group Assignment\covid-data\covid-data'
#   #test on folder containg all CSV files#    
   analyse(path_to_files)
